...
2. Create the Terraform Cloud project `aws` in the `sigass` organization if it does not already exist.
3. Create the Terraform Cloud workspace `aws-terraform-high-availability-wordpress` in that project.
4. In Terraform Cloud, add these workspace variables:
...
5. Move any sensitive values out of local `terraform.tfvars` if you want Terraform Cloud to be the source of truth for runs.
